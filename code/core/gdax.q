gook:([] time:`timestamp$();sym:`$();bids:();bsizes:();asks:();asizes:())           //schema for book table
...
rec.book:{[t;s]
  ...
  if[not bk~lb[s];                                                                  //compare to last book
     publish[`gook;enlist @[bk;`time`sym;:;("p"$t;s)]];                             //publish record if changed
...
getref:{[]
  r:.req.get["wss://ws-feed.pro.coinbase.com";()!()];                                //get reference data using reQ
...
.gdax.h:.ws.open["wss://ws-feed.pro.coinbase.com";`.gdax.upd]                               //open websocket to feed
\
