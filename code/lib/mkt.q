...
.mkt.getProduct24hrStats:{[sym]
  pid: .ref.getPID[sym];
  res: .cb.PC.get_product_24hr_stats[pid];
  stats: "F"$/:res;
  stats};
...
.mkt.getProductHistoricRates:{[sym;start;end;granularity];
  ...
  res: .cb.PC.get_product_historic_rates[pid; pykwargs kwargs];
...
.mkt.getProductTrades:{[sym;before;after;limit];
  ...
  if[switch 2;
    if[limit<100;
      kwargs[`limit]:limit]];
  res: .cb.PC.get_product_trades[pid; pykwargs kwargs];
...
.mkt.getProductOrderBook:{[sym;level]
  pid: .ref.getPID[sym];
  book: .cb.PC.get_product_order_book[pid; level];
