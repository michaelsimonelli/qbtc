// Implementation of p.bukowinski log4q: https://github.com/prodrive11/log4q
...
(` sv' ``lg,/:`$(),/:each[first;string lower key snk]) set' {{@[.lg.h[x]x;y;{[h;e]'"lg - ", string[h]," exception:",e}[x]]}[;l[x] p y]@/:snk[x]}@/: key[snk];n:(::);
...
INFO    [2012.03.01D23:44:01.593750000]:lg.q: This is a log 23 `adf (3;{x+y})
...
---------------
Logs pattern layout - format (.lg.fm) 
---------------
...
ERROR   [2012.03.01D23:32:30.609375000]:PID[1924];lg.q: simple message
q).lg.fm:"%c\t[%p]:H:%h;PID[%i];%d;%t;%f: %m\r\n"
q)ERROR ("%2 simple message";`another);
ERROR   [2012.03.01D23:34:30.234375000]:H:prodrive-notebo;PID[1924];2012.03.01;23:34:30.234;lg.q: %2 simple message
...
/add sink  
* file handle
  .lg.a[hopen `:my_test2.log;`INFO`ERROR]
* TCP handle with special modification function
  .lg.a[(hopen `::5555:user:pass;{x@(`upd;`msg;y)});`INFO`ERROR`FATAL]
...
ex:
  ...
  q lg.q -p 5001 -log info
  -----------
  q)INFO ("Test %1 log";1222);
  INFO    [2012.03.01D23:14:17.718750000]:lg.q: Test 1222 log
  ...
  ERROR | 2
  FATAL | 2
  q).lg.a[(hopen `::5555:user:pass;{x@(`upd;`msg;y)});`INFO`ERROR`FATAL]
  q).lg.snk
  SILENT| ,1
  DEBUG | ,1
  INFO  | 1 1800
  ...
  ERROR   [2012.03.01D23:15:22.609375000]:lg.q: Test 1222 log
  
  proc (5555)
  -----------
  q)(`msg;"ERROR\t[2012.03.01D23:15:22.609375000]:lg.q: Test 1222 log\r\n")

/remove sink
  .lg.r[1;`DEBUG`INFO] /removes logging to stdout at DEBUG and `INFO severity

q).lg.a[hopen `:my_test2.log;`INFO`ERROR]
q).lg.snk
SILENT| ,1
DEBUG | ,1
INFO  | 1 1800
